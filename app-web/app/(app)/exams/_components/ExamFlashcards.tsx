  ...
  if (isDone) {
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-16 flex flex-col items-center text-center gap-6">
        ...
        <div>
          <h2 className="text-xl font-semibold text-white">{t('exam.flashcards.sessionComplete')}</h2>
          <p className="text-sm text-white/45 mt-1">{t('exam.flashcards.reviewedAll').replace('{total}', String(total))}</p>
        </div>
        ...
        <div className="relative w-28 h-28">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-[10px] text-white/40 uppercase tracking-wide">{t('exam.flashcards.knewIt')}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 w-full max-w-xs">
          <div className="rounded-xl border border-green-500/20 bg-green-500/8 px-4 py-3 text-center">
            ...
            <p className="text-xs text-white/40 mt-0.5">{t('exam.flashcards.gotIt')}</p>
          </div>
          <div className="rounded-xl border border-orange-500/20 bg-orange-500/8 px-4 py-3 text-center">
            ...
            <p className="text-xs text-white/40 mt-0.5">{t('exam.flashcards.needReview')}</p>
          </div>
        </div>
          ...
          >
            {isSubmitting ? (
              <>
                ...
                {t('exam.flashcards.saving')}
              </>
            ) : (
              t('exam.flashcards.saveToStats')
            )}
          </button>
          <button
            type="button"
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between gap-4 mb-3">
          <p className="text-xs text-white/40 uppercase tracking-wider">
            {t('exam.flashcards.cardOf').replace('{current}', String(currentIndex + 1)).replace('{total}', String(total))}
          </p>
          <span className="text-xs text-white/40">
            {t('exam.flashcards.reviewed').replace('{evaluated}', String(evaluatedCount)).replace('{total}', String(total))}
          </span>
        </div>
          ...
          {questions.map((q, i) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => goTo(i)}
                title={`${t('exam.flashcards.card')} ${i + 1}`}
          ...
          >
            <div className="flex items-center justify-between">
              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-white/8 border border-white/12 text-white/50">
                {t('exam.flashcards.flashcardLabel')}
              </span>
              <span />
            </div>
            ...
            <div className="flex items-center justify-center gap-1.5 text-white/20">
              ...
              <span className="text-xs">{t('exam.flashcards.flipHint')}</span>
            </div>
          </div>
          ...
          >
            <div className="flex items-center justify-between">
              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-indigo-500/12 border border-indigo-500/20 text-indigo-400/70">
                {t('exam.flashcards.answerLabel')}
              </span>
              <span className="text-xs text-white/25">{t('exam.flashcards.howDidYouDo')}</span>
            </div>
      ...
      <div className="mt-6 flex items-center justify-between gap-3">
        ...
        {currentEval && (
          <span className={`text-xs font-medium px-3 py-1 rounded-full border ${
            ...
            {currentEval === 'got' ? t('exam.flashcards.gotIt') : t('exam.flashcards.needReview')}
          </span>
        )}
      ...
      <p className="mt-4 text-center text-xs text-white/20">
        {t('exam.flashcards.keyboardHint')}
      </p>
    </div>
  );
}
