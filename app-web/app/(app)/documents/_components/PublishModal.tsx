...
interface PublishModalProps {
  ...
  onClose: () => void;
  /** Called after a successful publish/unpublish so parent can reload */
...
export function PublishModal({
  ...
  const handleSuggestKeywords = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/documents/${documentId}/suggest-keywords`, {
        method: 'POST',
      });
      if (!res.ok) {
      ...
      <div className="relative w-full max-w-md bg-neutral-900 border border-white/15 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="min-h-[44px] bg-white/6 border border-white/12 rounded-lg px-3 py-2 flex flex-wrap gap-1.5 items-center
              ...
              {keywords.map((kw) => (
                <span
                  key={kw}
                  className="inline-flex items-center gap-1 bg-indigo-500/20 border border-indigo-400/30 text-indigo-300 rounded-full px-2 py-0.5 text-xs"
                >
                  {kw}
                  <button
        ...
        <div className="px-5 py-4 border-t border-white/10 flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            disabled={isSaving}
            className="flex-1 py-2 rounded-lg text-xs font-medium bg-indigo-500 hover:bg-indigo-600
