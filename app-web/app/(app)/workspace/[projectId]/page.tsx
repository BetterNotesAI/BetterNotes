...
import { saveOutputFile, type Project } from "@/lib/api";
import { incrementMessageCount } from "@/lib/api";
...
export default function ProjectWorkspace() {
    ...
    // Auth + project
    const { user, usageStatus, showPaywallModal, setShowPaywallModal, canSendMessage } = useWorkspaceAuth();
    ...
    useEffect(() => {
        if (outputFiles.some((o) => (o.content ?? "").trim())) {
            pendingAutoCompile.current = true;
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []); // intentionally once on mount via useOutputFiles load
    ...
    function replaceLastWorking(m: Msg[], newText: string): Msg[] {
        ...
        for (let i = copy.length - 1; i >= 0; i--) {
            if (copy[i].role === "assistant" && isTransientMsg(copy[i].content)) {
    ...
    async function saveAndCompile() {
        ...
        const savePromises = outputFiles.filter((f) => f.dirty).map((f) => saveOutputFile(projectId, f.filePath, f.content));
        const comp = await compileProject();
        if (comp.ok) markAllClean();
        ...
        <div className="flex h-screen text-white overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <div className="px-4 py-2.5 border-b border-white/8 flex items-center justify-between gap-3">
                    ...
                    {/* Center: PDF controls (preview only) */}
                    {activeTab === "preview" && pdfUrl && (
                        <div className="flex items-center gap-2">
                            <div className="flex items-center gap-1 rounded-lg border border-white/10 bg-white/[0.04] px-1 py-0.5">
                                <button onClick={() => setPdfZoom((z) => Math.max(50, z - 10))} className="h-6 w-6 flex items-center justify-center rounded text-white/50 hover:text-white/90 hover:bg-white/10 text-sm font-medium transition-colors" title="Zoom out">−</button>
                                <span className="text-xs text-white/40 w-10 text-center tabular-nums">{pdfZoom}%</span>
                                <button onClick={() => setPdfZoom((z) => Math.min(200, z + 10))} className="h-6 w-6 flex items-center justify-center rounded text-white/50 hover:text-white/90 hover:bg-white/10 text-sm font-medium transition-colors" title="Zoom in">+</button>
                            </div>
                            {pdfNumPages > 0 && (
                                <div className="flex items-center gap-1 rounded-lg border border-white/10 bg-white/[0.04] px-1 py-0.5">
                                    <button onClick={() => { const p = Math.max(1, pdfCurrentPage - 1); setPdfTargetPage(p); }} className="h-6 w-6 flex items-center justify-center rounded text-white/50 hover:text-white/90 hover:bg-white/10 transition-colors" title="Previous page">
                                    ...
                                    <input
                                        ...
                                        onBlur={() => setPageInput("")}
                                        onKeyDown={(e) => {
                                            if (e.key === "Enter") { const n = parseInt(pageInput, 10); if (!isNaN(n) && n >= 1 && n <= pdfNumPages) setPdfTargetPage(n); (e.target as HTMLInputElement).blur(); }
                            ...
                            {!outputFilesCollapsed && (
                                <div className="flex-1 overflow-y-auto py-1">
                                    {outputFiles.length === 0 ? (
                                        ...
                                        outputFiles.map((f) => (
                                            <div key={f.filePath} className={`group flex items-center gap-1.5 px-3 py-1.5 text-xs cursor-pointer ${f.filePath === activeOutputPath ? "bg-white/10 text-white" : "text-white/50 hover:bg-white/5 hover:text-white/70"}`}>
                                                ...
                                                {f.filePath !== "main.tex" && (
                                                    <button onClick={() => deleteOutputEntry(f.filePath, showConfirm, toast)} className="opacity-0 group-hover:opacity-100 text-white/25 hover:text-red-300 transition-opacity">
