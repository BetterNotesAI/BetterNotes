...
export function ProblemPublishModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t('problemSolver.publish.titleNew')}
    >
      {/* Backdrop */}
      <div
      ...
      <div className="relative w-full max-w-md bg-neutral-900 border border-white/15 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-white">
              {isAlreadyPublished ? t('problemSolver.publish.titleUpdate') : t('problemSolver.publish.titleNew')}
            </h2>
        ...
        <div className="px-5 py-4 space-y-4 flex-1">
          ...
          <div>
            <label className="block text-xs font-medium text-white/60 mb-1.5">
              {t('problemSolver.publish.university')} <span className="text-white/30">{t('problemSolver.publish.optional')}</span>
            </label>
            <input
              type="text"
              value={university}
              onChange={(e) => setUniversity(e.target.value)}
              placeholder={t('problemSolver.publish.universityPlaceholder')}
          ...
          <div>
            <label className="block text-xs font-medium text-white/60 mb-1.5">
              {t('problemSolver.publish.degree')} <span className="text-white/30">{t('problemSolver.publish.optional')}</span>
            </label>
            <input
              type="text"
              value={degree}
              onChange={(e) => setDegree(e.target.value)}
              placeholder={t('problemSolver.publish.degreePlaceholder')}
              className="w-full bg-white/6 border border-white/12 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/25 outline-none focus:border-orange-400/60 focus:ring-1 focus:ring-orange-400/30 transition-colors"
              maxLength={120}
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-medium text-white/60 mb-1.5">
              {t('problemSolver.publish.subject')} <span className="text-white/30">{t('problemSolver.publish.optional')}</span>
            </label>
            <input
              type="text"
              value={subject}
              onChange={(e) => setSubject(e.target.value)}
              placeholder={t('problemSolver.publish.subjectPlaceholder')}
              className="w-full bg-white/6 border border-white/12 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/25 outline-none focus:border-orange-400/60 focus:ring-1 focus:ring-orange-400/30 transition-colors"
              maxLength={120}
            />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-medium text-white/60">
                {t('problemSolver.publish.keywords')} <span className="text-white/30">{t('problemSolver.publish.optional')}</span>
              </label>
              <button
              ...
              >
                {isSuggestingKeywords ? (
                  <>
                    ...
                    {t('problemSolver.publish.suggesting')}
                  </>
                ) : (
                  <>
                    ...
                    {t('problemSolver.publish.suggestWithAI')}
                  </>
                )}
              </button>
            </div>
            ...
            <div className="min-h-[44px] bg-white/6 border border-white/12 rounded-lg px-3 py-2 flex flex-wrap gap-1.5 items-center focus-within:border-orange-400/60 focus-within:ring-1 focus-within:ring-orange-400/30 transition-colors cursor-text">
              {keywords.map((kw) => (
                ...
                >
                  ...
                  <button
                    ...
                    aria-label={t('problemSolver.publish.removeKeyword', { kw })}
                  >
                    ×
                  </button>
                </span>
              ...
              <input
                ...
                placeholder={keywords.length === 0 ? t('problemSolver.publish.keywordPlaceholder') : ''}
                className="flex-1 min-w-[100px] bg-transparent text-sm text-white placeholder:text-white/25 outline-none"
              />
            </div>
            <p className="text-[10px] text-white/30 mt-1">
              {t('problemSolver.publish.keywordHint')}
            </p>
          </div>

          {/* Visibility */}
          <div>
            <label className="block text-xs font-medium text-white/60 mb-1.5">{t('problemSolver.publish.visibility')}</label>
            <div className="flex gap-2">
              {(['private', 'public'] as const).map((v) => (
                ...
                >
                  {v === 'private' ? t('problemSolver.publish.visibilityPrivate') : t('problemSolver.publish.visibilityPublic')}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="px-5 py-4 border-t border-white/10 flex items-center gap-2 shrink-0">
          {isAlreadyPublished && (
            ...
            >
              {t('problemSolver.publish.unpublish')}
            </button>
          )}
          <button
            onClick={onClose}
          ...
          >
            {t('problemSolver.publish.cancel')}
          </button>
          <button
          ...
          >
            {isSaving ? (
              <>
                <span className="animate-spin inline-block">⟳</span>
                {t('problemSolver.publish.saving')}
              </>
            ) : isAlreadyPublished ? (
              t('problemSolver.publish.update')
            ) : (
              t('problemSolver.publish.publish')
            )}
          </button>
        </div>
      </div>
