...
import { deleteProject, starProject, duplicateProject, updateProject, type Project, type Folder } from "@/lib/api";
...
export default function ProjectCard({ project, folders, onUpdate }: ProjectCardProps) {
    ...
    return (
        <>
            ...
            className={`group relative rounded-2xl border transition-all duration-200 ${
                isDragging
                    ? "border-purple-400/40 bg-purple-400/5 opacity-50 scale-95 cursor-grabbing"
                    : "border-white/8 bg-white/[0.03] hover:bg-white/[0.06] hover:border-white/15 cursor-grab"
            }`}
        >
                {/* Cover area */}
                <Link href={`/workspace/${project.id}`} className="block">
                    <div className="h-32 rounded-t-2xl overflow-hidden bg-gradient-to-br from-purple-500/10 via-blue-500/5 to-transparent flex items-center justify-center">
                ...
                <div ref={menuRef} className="absolute top-2 right-2">
                    ...
                    {menuOpen && (
                        <div className="absolute right-0 mt-1 w-40 rounded-xl border border-white/15 bg-neutral-900/95 backdrop-blur shadow-xl py-1 z-50">
                            ...
                            <button onClick={() => { setMenuOpen(false); setShowMoveModal(true); }} className="w-full px-3 py-2 text-left text-sm text-white/80 hover:bg-white/10 flex items-center gap-2">
