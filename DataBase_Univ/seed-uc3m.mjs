 ...
 *
 * Reads baseDatosUC3M_selenium.json and inserts:
 *   1 university → 177 degree programmes → 7,625 courses
 *
 * Usage:
 *   SUPABASE_URL=https://xxx.supabase.co \
 *   SUPABASE_SERVICE_ROLE_KEY=eyJ... \
 *   node DataBase_Univ/seed-uc3m.mjs
 *
...
// ── Main ─────────────────────────────────────────────────────
async function main() {
  const raw = readFileSync(join(__dirname, 'baseDatosUC3M_selenium.json'), 'utf8');
  ...
  // ── 2. Build degree programmes ────────────────────────────
  // Slugs must be unique per university — deduplicate by appending index if needed.
  const slugCount = {};
  ...
  // Upsert programs and get their IDs back
  ...
  // Map slug → id for course linking
  const programIdBySlug = Object.fromEntries(insertedPrograms.map((r) => [r.slug, r.id]));
  // Also rebuild slug array in same order as programRows for reliable matching
  const programIdByIndex = insertedPrograms.map((_, i) => insertedPrograms[i]?.id);

  // Re-fetch in insertion order using the slug map built from programRows
  ...
  uniData.programas.forEach((p, pIdx) => {
    ...
    for (const [yearKey, yearData] of Object.entries(p.plan_estudios)) {
      ...
      for (const [semKey, courses] of Object.entries(yearData)) {
        const semester = parseInt(semKey.replace('cuatrimestre_', ''), 10);
